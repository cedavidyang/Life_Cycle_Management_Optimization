...
def beamWidthVariable(rebar_type=None):
    if rebar_type is None:
        from corrosion.initialVariables import rebar_type
    if 'd' in rebar_type.lower():
        mu = DECK_WIDTH_MEAN
        sigma = mu * DECK_WIDTH_COV
        if DECK_WIDTH_DISTR == 'deterministic':
            beam_width = Deterministic('beam_width', mu, sigma)
        elif DECK_WIDTH_DISTR == 'lognormal':
...
def beamDepthVariable(rebar_type=None):
    if rebar_type is None:
        from corrosion.initialVariables import rebar_type
    if 'd' in rebar_type.lower():
        mu = DECK_DEPTH_MEAN
        sigma = mu * DECK_DEPTH_COV
        if DECK_DEPTH_DISTR == 'deterministic':
            beam_depth = Deterministic('beam_depth', mu, sigma)
        elif DECK_DEPTH_DISTR == 'normal':
...
def flangeWidthVariable(rebar_type=None):
    if rebar_type is None:
        from corrosion.initialVariables import rebar_type
    if 'd' in rebar_type.lower():
        mu = DECK_WIDTH_MEAN
        ...
        if FLANGE_WIDTH_DISTR == 'deterministic':
            flange_width = Deterministic('flange_width', mu, sigma)
        elif FLANGE_WIDTH_DISTR == 'lognormal':
            flange_width = Lognormal('flange_width', mu, sigma)

    return flange_width


def flangeDepthVariable(rebar_type=None):
    if rebar_type is None:
        from corrosion.initialVariables import rebar_type
    if 'd' in rebar_type.lower():
        mu = DECK_DEPTH_MEAN
        ...
        if FLANGE_THICK_DISTR == 'deterministic':
            flange_depth = Deterministic('flange_depth', mu, sigma)
        elif FLANGE_THICK_DISTR == 'normal':
            flange_depth = Normal('flange_depth', mu, sigma)
...
def assembleBeamDict(ME_flex, ME_shear, fc, lbda_fc, fy, fyv, Ast, Asvt, b, d, bf, hf, dv, sv):
    ME_dict = {'ME_flex': ME_flex, 'ME_shear':ME_shear}
    material_dict = {'fc':fc, 'lbda_fc':lbda_fc, 'fy':fy, 'fyv':fyv}
...
def assembleFrpBeamDict(ME_dict, material_dict, geo_dict, ME_flex, ME_shear,
                        ft, fcuk, ecu, Es, fyc, Efrp, ffrp, tfrp, bfrp, fanchor, eini,
                        Efrpv, tfrpv, ffrpv, strform,
                        h, a, ss, l, dc, Asc, dfrp, dfrpt, barType, dsv, wfrpv, sfrpv, frpIncline, env):
