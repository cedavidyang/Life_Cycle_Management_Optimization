...
#DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_anchor', 'no_evidence_shear')
#DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_anchor', 'evidence_condition_state_shear')
#DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_anchor', 'no_evidence')
#DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_anchor', 'evidence_condition_state')

#DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_bond', 'no_evidence_shear')
#DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_bond', 'evidence_condition_state_shear')
#DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_bond', 'no_evidence')
#DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_bond', 'evidence_condition_state')

#DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_anchor_degrade', 'no_evidence_shear')
#DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_anchor_degrade', 'evidence_condition_state_shear')
#DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_anchor_degrade', 'no_evidence')
#DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_anchor_degrade', 'evidence_condition_state')

#DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_bond_degrade', 'no_evidence_shear')
DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_bond_degrade', 'evidence_condition_state_shear')
#DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_bond_degrade', 'no_evidence')
#DATAFILE_PATH = os.path.join(os.path.abspath('./'), 'data', 'frp_U_bond_degrade', 'evidence_condition_state')
...
def main():
    ...
    mat = scipy.io.loadmat( os.path.join(os.path.abspath('./'), 'matlab_data', 'frp_degradation.mat') )
    tdegrade = mat['timeEnvYR'].flatten()
    sfrpdegrade = mat['degradeFrp2Env'].flatten()
    frp_interp1d = interp1d(tdegrade, sfrpdegrade, bounds_error=False, fill_value=sfrpdegrade[-1])
    ...
    while weight_sum <= SUM_WEIGHT and n_iter<=MAX_ITER_LW: 
        ...
        #frp_flexure_history = np.zeros((service_time.size, N_SMP))
        #frp_shear_history = np.zeros((service_time.size, N_SMP))
        ...
        for age in service_time:
            ...
            if age < EM_SHEAR_YR:
                ...
                if WFRPV_MEAN/SFRPV_MEAN == 1 and age == EM_SHEAR_YR:
                    ds_at_str = np.copy(ds_smp)
                    dsv_smp = ds_at_str
                elif WFRPV_MEAN/SFRPV_MEAN == 1 and age > EM_SHEAR_YR:
                    dsv_smp = ds_at_str
                else:
                    dsv_smp = ds_smp

                tAfterStr = age - EM_SHEAR_YR
                if EM_SHEAR_FORM.lower() == 'w':
                    # FRP degradation
                    ffrpvt_smp = frp_degrade(tAfterStr) * ffrpv_smp
                    env_smp = np.ones(N_SMP)
                else:
                    # bonding degradation
                    env_smp = bond_degrade(tAfterStr) * np.ones(N_SMP)
                ...
                ME_dict, material_dict, geo_dict = assembleFrpBeamDict(ME_dict, material_dict, geo_dict, ME_flex_frp_smp, ME_shear_frp_smp,
                        ...
                        h_smp, a_smp, ss_smp, l_smp, dsc_smp, Asc_smp, dfrp_smp, dfrpt_smp, BAR_TYPE, dsv_smp, wfrpv_smp, sfrpv_smp, frpIncline_smp, env_smp)
                frpBeam = FRPBeam('frp_beam', N_SMP, ME_dict, material_dict, geo_dict)
                vu_smp = frpBeam.shearCapacity()
                rc_shear_history[service_time == age, :] = vu_smp
