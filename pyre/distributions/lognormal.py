...
class Lognormal(Distribution):
  ...
  def __init__(self, name,mean,stdv,input_type=None,startpoint=None):
    ...
    # p1, p2, p3, p4 = lamb(logMean), zeta(logStd), 0, 0
    ...
    # lognorm(logStd, scale=exp(logMean))
    # self.rv = stats.lognorm(p1, loc=p2, scale=p3)
    self.rv = stats.lognorm(p2, scale=np.exp(p1))
  ...
  def setMarginalDistribution(self):
    ...
    if self.input_type == None:
      ...
      stdv = self.stdv
      cov = stdv / mean
      zeta = np.sqrt(np.log(1+cov**2)) # shape factor (logStd)
      lamb = np.log(mean) - 0.5*zeta**2 # logMean
      p1 = lamb
      p2 = zeta
      p3 = 0
      p4 = 0
      ...
      lamb = self.mean
      zeta = self.stdv
      self.mean = np.exp(lamb+0.5*(zeta**2))
      self.stdv = np.exp(lamb+0.5*(zeta**2)) * (np.exp(zeta**2)-1)**0.5
      p1 = lamb
      p2 = zeta
      p3 = 0
      p4 = 0
    return self.mean, self.stdv, p1, p2, p3,p4
